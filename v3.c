...
void v3_sendnames(struct sftpjob *job, 
  ...
  while(nnames > 0) {
    ...
    if(names->valid) {
       ...
       */
      p = perms;
      *p++ = typedetails[names->type];
      *p++ = (names->permissions & 00400) ? 'r' : '-';
      *p++ = (names->permissions & 00200) ? 'w' : '-';
      switch(names->permissions & 04100) {
      case 00000: *p++ = '-'; break;
      case 00100: *p++ = 'x'; break;
      case 04000: *p++ = 'S'; break;
      case 04100: *p++ = 's'; break;
      }
      *p++ = (names->permissions & 00040) ? 'r' : '-';
      *p++ = (names->permissions & 00020) ? 'w' : '-';
      switch(names->permissions & 02010) {
      case 00000: *p++ = '-'; break;
      case 00010: *p++ = 'x'; break;
      case 02000: *p++ = 'S'; break;
      case 02010: *p++ = 's'; break;
      }
      *p++ = (names->permissions & 00004) ? 'r' : '-';
      *p++ = (names->permissions & 00002) ? 'w' : '-';
      switch(names->permissions & 02001) {
      case 00000: *p++ = '-'; break;
      ...
      *p = 0;
      sprintf(linkcount, "%"PRIu32, names->link_count);
      sprintf(size, "%"PRIu64, names->size);
      m = names->mtime.seconds;
      gmtime_r(&m, &mtime);
      ...
      longname = alloc(job->a, 80 + strlen(names->name));
      /* The draft is pretty specific about field widths */
      sprintf(longname, "%10.10s %3.3s %-8.8s %-8.8s %8.8s %12.12s %s",
              perms, linkcount, names->owner, names->group,
              size, date, names->name);
...
void sftp_readdir(struct sftpjob *job) {
  ...
  for(n = 0; n < MAXNAMES;) {
    ...
    strcat(fullpath, "/");
    strcat(fullpath, d[n].name);
    if(lstat(fullpath, &sb)) {
      send_errno_status(job);
      return;
    }
    stat_to_attrs(job->a, &sb, &d[n]);
    d[n].name = childpath;
    ++n;
  }
...
void sftp_v3_realpath(struct sftpjob *job) {
  ...
  attr.name = alloc(job->a, PATH_MAX + 64);
  if(realpath(path, attr.name)) {
